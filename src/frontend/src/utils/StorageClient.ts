...
// All HTTP errors on this platform are transient — always retry
...
// Retry ALL errors — 400/500 are transient on this platform
function isRetriableError(_error: any): boolean {
  return true;
}
...
// Keep isRetriableError exported for tests
...
export class StorageClient {
  ...
  /**
   * putBlob — fault-tolerant chunked upload with:
   * - 5 MB chunks (sequential, not parallel)
   * - 30s per-chunk timeout via AbortController
   * - Max 5 retries per chunk with exponential backoff
   * - Skips already-completed chunks (resume support)
   * - Retries ALL errors including 400/500 (transient on this platform)
   */
  public async putBlob(
    ...
    // Build chunks + compute all hashes (including skipped ones — needed for tree)
    const chunks = this.createFileChunks(file, CHUNK_SIZE);
    ...
    // Build blob hash tree from ALL chunk hashes
    ...
    // Get certificate and upload blob tree
    ...
    // Upload each chunk sequentially with per-chunk retry
    for (let i = 0; i < totalChunks; i++) {
      // Skip already-completed chunks but count for progress
      ...
      // Per-chunk retry loop with 30s timeout
      let chunkAttempt = 0;
      let chunkSuccess = false;

      while (chunkAttempt <= MAX_RETRIES && !chunkSuccess) {
        try {
          // 30s timeout per chunk attempt
          ...
          try {
            ...
            const queryParams = new URLSearchParams({
              owner_id: this.backendCanisterId,
              blob_hash: hashString,
              chunk_hash: chunkHash.toShaString(),
              chunk_index: i.toString(),
              bucket_name: this.bucket,
              project_id: this.projectId,
            });
            const url = `${this.storageGatewayClient.getStorageGatewayUrl()}/${GATEWAY_VERSION}/chunk/?${queryParams.toString()}`;
            ...
            if (!response.ok) {
              const errorText = await response.text();
              throw new Error(
                `Chunk ${i} HTTP ${response.status}: ${errorText}`,
              );
            }
          ...
          const delay = Math.min(
            BASE_DELAY_MS * 2 ** (chunkAttempt - 1) + Math.random() * 1000,
            MAX_DELAY_MS,
          );
          console.warn(
            `Chunk ${i} failed (attempt ${chunkAttempt}/${MAX_RETRIES + 1}), retrying in ${Math.round(delay)}ms...`,
          );
